...
In Section~\ref{rg:sec:performance} we detail a plethora of benchmarks
...
%---we get results that are as accurate as the brick size.
...
% any renderer that uses a volume pool would share this code.  The
...
% The depth is the only parameter that varies
% \emph{while} we progress along a ray.  Moreover, the appropriate
% sampling rate is aliased to the relatively few levels of detail that
...
% derive frustum and viewing angles that result in the simultaneous
...
% {This paragraph feels really out of place here, but we need something
...
typical case with the RMI data set (see Section~\ref{rg:sec:data}), time
...
% that have not been thoroughly addressed in previous literature.  We
...
% {this is too long. important message: rebricking is a good idea,
...
%% since we need space.  guess it got compressed out ;-)
...
% 
% {make it clear we are compressing each brick in isolation, storing
...
%   factor of less than half their original size.}
